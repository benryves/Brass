...
namespace Brass {
    ...
    partial class Program {
        ...
        public static Hashtable AvailableMacros;
        ...
        public class MacroReplacement : IComparable {
            ...
            public void GenerateSignature() {
                this.Signature = new string[this.Arguments.Count];
                for (int i = 0; i < this.Arguments.Count; i++) {
                    this.Arguments[i] = ((string)this.Arguments[i]).Trim();
                    if (((string)this.Arguments[i]).StartsWith("{") && ((string)this.Arguments[i]).EndsWith("}")) {
                        this.Signature[i] = ((string)this.Arguments[i]).Substring(1, ((string)this.Arguments[i]).Length - 2).Trim().ToLower();
                    } else {
                        this.Signature[i] = "*";
                    }
                }
            ...
            public int CompareTo(object other) {
                ...
                if (that.Signature.Length == this.Signature.Length) {
                    ...
                    for (int i = 0; i < this.Signature.Length; i++) {
                        if (this.Signature[i] == "*") ++ThisWildcard;
                        if (that.Signature[i] == "*") ++ThatWildcard;
                    }
        ...
        public static void AddMacroThroughDefinition(string MacroDefinition, string Filename, int LineNumber, bool SecondPass) {
            ...
            // Extract the macro name
            MacroName = MacroName.Trim().ToLower();
            if (MacroName == "") {
            ...
            object ExistingMacro = AvailableMacros[MacroName];
            if (ExistingMacro == null) {
                // Add a new macro and add the replacement
                Macro AddNew = new Macro(MacroName);
                AddNew.Replacements.Add(Replacement);
                AvailableMacros[MacroName] = AddNew;
                LastMacro = AddNew;
            } else {
                // Add to an existing macro:
                Macro OldMacro = (Macro)ExistingMacro;
                // Check for a duplicate macro signature:
                ...
                if (RestOfLine.Trim().StartsWith("(")) {
                    ...
                    if (ArgumentListEnd != -1) {
                        ...
                        for (int i = 0; i < Arguments.Length; ++i) {
                            Arguments[i] = (TranslateArgument(TokenedReplacement(Arguments[i].Trim(), true, null, null), true)).ToString();
        ...
        public static string[] GetArguments(ref string ArgumentString) {
            int ArgumentListEnd = 0;
            if (ArgumentString.Trim().StartsWith("(")) {
